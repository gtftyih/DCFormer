...
def b_mpjpe(predicted, target): ##bone loss
    assert predicted.shape == target.shape
    b, f, n, c = predicted.shape
    predicted_bones, target_bones = torch.zeros(b, f, n-1, c), torch.zeros(b, f, n-1, c)
    bones = [(0, 1), (1, 2), (2, 3), (0, 4), (4, 5), (5, 6), (0, 7), (7, 8), (8, 9), (9, 10), (8, 11), (11, 12), (12, 13), (8, 14), (14, 15), (15, 16)]
    for n_bone in range(len(bones)):
        predicted_bones[:, :, n_bone] = predicted[:, :, bones[n_bone][0]] - predicted[:, :, bones[n_bone][1]]
        target_bones[:, :, n_bone] = target[:, :, bones[n_bone][0]] - target[:, :, bones[n_bone][1]]
    return mpjpe_cal(predicted_bones, target_bones)

def b_diff(predicted):
    b, f, n, c = predicted.shape
    diff = 0
    for i in range(b):
        for j in range(f):
            diff += torch.norm(torch.norm(predicted[i,j,1]-predicted[i,j,2])-torch.norm(predicted[i,j,4]-predicted[i,j,5])) + torch.norm(torch.norm(predicted[i,j,2]-predicted[i,j,3])-torch.norm(predicted[i,j,5]-predicted[i,j,6])) + torch.norm(torch.norm(predicted[i,j,14]-predicted[i,j,15])-torch.norm(predicted[i,j,11]-predicted[i,j,12])) + torch.norm(torch.norm(predicted[i,j,12]-predicted[i,j,13])-torch.norm(predicted[i,j,15]-predicted[i,j,16])) + torch.norm(torch.norm(predicted[i,j,8]-predicted[i,j,14])-torch.norm(predicted[i,j,8]-predicted[i,j,11])) + torch.norm(torch.norm(predicted[i,j,0]-predicted[i,j,1])-torch.norm(predicted[i,j,0]-predicted[i,j,4]))
    return diff / (b*f)
