...
from common.gendb_tools import gen_camera_fetch

class Fusion(data.Dataset):
    def __init__(self, opt, dataset, root_path, train=True, MAE=False, tds=1, use_camera=False, keypoint=None):
        self.data_type = opt.dataset
        self.train = train
        if keypoint == None:
            self.keypoints_name = opt.keypoints
        else:
            self.keypoints_name = keypoint
        ...
        if self.train:
            ...
            self.cameras_test, self.poses_test, self.poses_test_2d = self.fetch(dataset, self.test_list,
                                                                                subset=self.subset)
            self.generator = ChunkedGenerator(opt.batchSize // opt.stride, self.cameras_test, self.poses_test,  #batch_size=opt.batchSize//opt.stride
                                              self.poses_test_2d, 
        ...
        for subject in folder_list:
            for action in dataset[subject].keys():
                ...
                for cam in anim['cameras']:
                    ...
                    if self.use_camera:
                        #print(pos_3d.shape) #_, 17, 3
                        pos_3d = gen_camera_fetch(pos_3d, cam) ##camera_to_image_frame
                ...
                #print(len(positions_3d)) #4
                #print(len(positions_3d[0]), len(anim['positions'])) #equal：_, 17, 3
        ...
        for subject in subjects:
            for action in self.keypoints[subject].keys():
                ...
                if subject in dataset.cameras():
                    ...
                    #print(len(cams), len(poses_2d)) #4, 4
                ...
                if parse_3d_poses and 'positions_3d' in dataset[subject][action]:
                    ...
                    #print(len(poses_3d), len(poses_2d)) #4, 4
                    #print(poses_3d[0][0][0], poses_2d[0][0][0])
