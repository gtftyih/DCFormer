...
class opts():
    ...
    def init(self):
        ...
        self.parser.add_argument('--root_path', type=str, default='/home/cheng/DCFormer/dataset/')
        ...
        self.parser.add_argument('--nepoch', type=int, default=1000000)
        self.parser.add_argument('-b','--batchSize', type=int, default=128)
        self.parser.add_argument('--lr', type=float, default=1e-3) #1e-3
        ...
        self.parser.add_argument('--workers', type=int, default=8) #8
        self.parser.add_argument('-lrd', '--lr_decay', default=0.96, type=float) #0.96
        ...
        self.parser.add_argument('--manualSeed', type=int, default=42)

        self.parser.add_argument('--MAE', action='store_true')
        self.parser.add_argument('-tmr','--temporal_mask_rate', type=float, default=0)
        self.parser.add_argument('-tmr_l','--temporal_mask_rate_location', type=int, default=2)
        self.parser.add_argument('-tmr_n','--temporal_mask_rate_nums', type=int, default=81)
        self.parser.add_argument('-smn', '--spatial_mask_num', type=int, default=1)
        self.parser.add_argument('-smn_r', '--spatial_mask_num_rands', type=int, default=10)
        ...
        ##another##
        self.parser.add_argument('--previous_dir_scale', type=str, default='')
        self.parser.add_argument('--previous_dir_depth', type=str, default='')
        self.parser.add_argument('--previous_dir_agg', type=str, default='')
        self.parser.add_argument('--layers_pre', type=str, default=6)
        self.parser.add_argument('--pre_train', action='store_true')
        self.parser.add_argument("--local_rank", default=os.getenv('LOCAL_RANK', -1), type=int)
        self.parser.add_argument('-acc', '--accumulation_steps', type=int, default=2)
        self.parser.add_argument('--use_camera', action='store_true')
